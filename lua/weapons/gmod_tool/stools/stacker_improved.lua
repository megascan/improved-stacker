...
local cvarMaxPerPlayer = CreateConVar("improved_stacker_max_per_player", oldMaxTotal:GetInt(), cvarFlags, "Defines the max amount of props that a player can have spawned from stacker")
local cvarMaxPerStack = CreateConVar("improved_stacker_max_per_stack", oldMaxCount:GetInt(), cvarFlags, "Defines the max amount of props that can be stacked at a time")
local cvarDelay = CreateConVar("improved_stacker_delay", oldDelay:GetFloat(), cvarFlags, "Determines the amount of time that must pass before a player can use stacker again")
local cvarMaxOffX = CreateConVar("improved_stacker_max_offsetx", oldMaxOffX:GetFloat(), cvarFlags, "Defines the max distance on the x plane that stacked props can be offset (for individual control)")
local cvarMaxOffY = CreateConVar("improved_stacker_max_offsety", oldMaxOffY:GetFloat(), cvarFlags, "Defines the max distance on the y plane that stacked props can be offset (for individual control)")
local cvarMaxOffZ = CreateConVar("improved_stacker_max_offsetz", oldMaxOffZ:GetFloat(), cvarFlags, "Defines the max distance on the z plane that stacked props can be offset (for individual control)")
...
elseif SERVER then
	...
	util.AddNetworkString("improved_stacker_error")
	...
	function TOOL:SendError(str)
		net.Start("improved_stacker_error")
		net.WriteString(str)
...
--]]
--
function TOOL:GetMaxPerPlayer()
	return cvarMaxPerPlayer:GetInt()
end

function TOOL:GetNumberPlayerEnts()
	return improvedstacker.GetEntCount(self:GetOwner(), 0)
end
...
function TOOL:GetOffsetX()
	return math.Clamp(self:GetClientNumber("offsetx"), -cvarMaxOffX:GetFloat(), cvarMaxOffX:GetFloat())
end

function TOOL:GetOffsetY()
	return math.Clamp(self:GetClientNumber("offsety"), -cvarMaxOffY:GetFloat(), cvarMaxOffY:GetFloat())
end

function TOOL:GetOffsetZ()
	return math.Clamp(self:GetClientNumber("offsetz"), -cvarMaxOffZ:GetFloat(), cvarMaxOffZ:GetFloat())
end
...
function TOOL:GetRotateP()
	return math.Clamp(self:GetClientNumber("pitch"), -MAX_ANGLE, MAX_ANGLE)
end

function TOOL:GetRotateY()
	return math.Clamp(self:GetClientNumber("yaw"), -MAX_ANGLE, MAX_ANGLE)
end

function TOOL:GetRotateR()
	return math.Clamp(self:GetClientNumber("roll"), -MAX_ANGLE, MAX_ANGLE)
end

function TOOL:GetRotationAngle()
...
--]]
--
function TOOL:ShouldApplyFreeze()
	return self:GetClientNumber("freeze") == 1
end

function TOOL:ShouldForceFreeze()
	return cvarFreeze:GetBool()
end
...
--]]
--
function TOOL:ShouldApplyWeld()
	return self:GetClientNumber("weld") == 1
end

function TOOL:ShouldForceWeld()
	return cvarWeld:GetBool()
end
...
--]]
--
function TOOL:ShouldApplyNoCollide()
	return self:GetClientNumber("nocollide") == 1
end

function TOOL:ShouldForceNoCollide()
	return cvarNoCollide:GetBool()
end
...
function TOOL:ShouldStackRelative()
	return self:GetClientNumber("relative") == 1
end
...
--]]
--
function TOOL:ShouldGhostAll()
	return self:GetClientNumber("ghostall") == 1
end
...
--]]
--
function TOOL:ShouldAddHalos()
	return self:GetClientNumber("draw_halos") == 1
end

function TOOL:GetHaloR()
	return math.Clamp(self:GetClientNumber("halo_r"), 0, 255)
end

function TOOL:GetHaloG()
	return math.Clamp(self:GetClientNumber("halo_g"), 0, 255)
end

function TOOL:GetHaloB()
	return math.Clamp(self:GetClientNumber("halo_b"), 0, 255)
end

function TOOL:GetHaloA()
	return math.Clamp(self:GetClientNumber("halo_a"), 0, 255)
end
...
--]]
--
function TOOL:GetUseShiftKey()
	return self:GetClientNumber("use_shift_key") == 1
end
...
function TOOL:LeftClick(tr, isRightClick)
	...
	if ply:KeyDown(IN_USE) or self:GetUseShiftKey() and ply:KeyDown(IN_SPEED) then
		...
		ply:ConCommand("improved_stacker_count " .. newCount)
	...
	for i = 1, count do
		...
		self:ApplyPhysicalProperties(ent, newEnt, tr.PhysicsBone, {
			GravityToggle = physGrav,
			Material = physMat
		})
...
function TOOL:Reload()
	...
	ply:ConCommand("improved_stacker_direction " .. direction)
...
function TOOL:ApplyMaterial(ent, material)
	if not self:ShouldApplyMaterial() then
		ent:SetMaterial("")
...
function TOOL:ApplyPhysicalProperties(original, newEnt, boneID, properties)
	...
	if boneID then
		construct.SetPhysProp(nil, newEnt, boneID, nil, properties)
	end
...
if CLIENT then
	...
	local cvarCount = GetConVar("improved_stacker_count")
	local cvarMode = GetConVar("improved_stacker_mode")
	local cvarDirection = GetConVar("improved_stacker_direction")
	local cvarOffsetX = GetConVar("improved_stacker_offsetx")
	local cvarOffsetY = GetConVar("improved_stacker_offsety")
	local cvarOffsetZ = GetConVar("improved_stacker_offsetz")
	local cvarPitch = GetConVar("improved_stacker_pitch")
	local cvarYaw = GetConVar("improved_stacker_yaw")
	local cvarRoll = GetConVar("improved_stacker_roll")
	local cvarRelative = GetConVar("improved_stacker_relative")
	local cvarMaterial = GetConVar("improved_stacker_material")
	local cvarColor = GetConVar("improved_stacker_color")
	local cvarGhostAll = GetConVar("improved_stacker_ghostall")
	local cvarOpacity = GetConVar("improved_stacker_opacity")
	local cvarHalo = GetConVar("improved_stacker_draw_halos")
	local cvarHaloR = GetConVar("improved_stacker_halo_r")
	local cvarHaloG = GetConVar("improved_stacker_halo_g")
	local cvarHaloB = GetConVar("improved_stacker_halo_b")
	local cvarHaloA = GetConVar("improved_stacker_halo_a")
	local cvarHalo = GetConVar("improved_stacker_draw_halos")
	local cvarAxis = GetConVar("improved_stacker_draw_axis")
	local cvarAxisLbl = GetConVar("improved_stacker_axis_labels")
	local cvarAxisAng = GetConVar("improved_stacker_axis_angles")
	...
	surface.CreateFont("improved_stacker_direction", {
		font = "Arial",
		size = 24,
		weight = 700,
		antialias = true
	...
	local function getDirection()
		return cvarDirection:GetInt()
	end

	local function getOpacity()
		return cvarOpacity:GetInt()
	end

	local function shouldGhostAll()
		return cvarGhostAll:GetBool()
	end

	local function shouldStackRelative()
		return cvarRelative:GetBool()
	end

	local function shouldApplyMaterial()
		return cvarMaterial:GetBool()
	end

	local function shouldApplyColor()
		return cvarColor:GetBool()
	end

	local function shouldAddHalos()
		return cvarHalo:GetBool()
	end

	local function getOffsetVector()
		return Vector(math.Clamp(cvarOffsetX:GetFloat(), -cvarMaxOffX:GetFloat(), cvarMaxOffX:GetFloat()), math.Clamp(cvarOffsetY:GetFloat(), -cvarMaxOffY:GetFloat(), cvarMaxOffY:GetFloat()), math.Clamp(cvarOffsetZ:GetFloat(), -cvarMaxOffZ:GetFloat(), cvarMaxOffZ:GetFloat()))
	end

	local function getRotationAngle()
		return Angle(math.Clamp(cvarPitch:GetFloat(), -MAX_ANGLE, MAX_ANGLE), math.Clamp(cvarYaw:GetFloat(), -MAX_ANGLE, MAX_ANGLE), math.Clamp(cvarRoll:GetFloat(), -MAX_ANGLE, MAX_ANGLE))
	end

	local function getHaloColor()
		return Color(cvarHaloR:GetInt(), cvarHaloG:GetInt(), cvarHaloB:GetInt(), cvarHaloA:GetInt())
	end
	...
	function TOOL:Init()
		...
		cvarCount = GetConVar("improved_stacker_count")
		cvarMode = GetConVar("improved_stacker_mode")
		cvarDirection = GetConVar("improved_stacker_direction")
		cvarOffsetX = GetConVar("improved_stacker_offsetx")
		cvarOffsetY = GetConVar("improved_stacker_offsety")
		cvarOffsetZ = GetConVar("improved_stacker_offsetz")
		cvarPitch = GetConVar("improved_stacker_pitch")
		cvarYaw = GetConVar("improved_stacker_yaw")
		cvarRoll = GetConVar("improved_stacker_roll")
		cvarRelative = GetConVar("improved_stacker_relative")
		cvarMaterial = GetConVar("improved_stacker_material")
		cvarColor = GetConVar("improved_stacker_color")
		cvarGhostAll = GetConVar("improved_stacker_ghostall")
		cvarOpacity = GetConVar("improved_stacker_opacity")
		cvarHalo = GetConVar("improved_stacker_draw_halos")
		cvarHaloR = GetConVar("improved_stacker_halo_r")
		cvarHaloG = GetConVar("improved_stacker_halo_g")
		cvarHaloB = GetConVar("improved_stacker_halo_b")
		cvarHaloA = GetConVar("improved_stacker_halo_a")
		cvarHalo = GetConVar("improved_stacker_draw_halos")
		cvarAxis = GetConVar("improved_stacker_draw_axis")
		cvarAxisLbl = GetConVar("improved_stacker_axis_labels")
		cvarAxisAng = GetConVar("improved_stacker_axis_angles")
	end
	...
	local function createGhostStack(ent)
		if improvedstacker.GetGhosts() then
		...
		if not shouldGhostAll() and count ~= 0 then
			count = 1
		end
	...
	local function validateGhostStack()
		...
		if #ghosts ~= count and shouldGhostAll() then
			return false
		elseif #ghosts ~= 1 and not shouldGhostAll() then
			return false
		end
	...
	--]]
	--
	hook.Add("PreDrawHalos", "improved_stacker_predrawhalos", function()
	...
	hook.Add("PostDrawTranslucentRenderables", "improved_stacker_directions", function(drawingDepth, drawingSky)
		...
		-- check if they have the toolgun out and have stacker selected
		local wep = ply:GetActiveWeapon()
		if not (IsValid(wep) and wep:GetClass() == "gmod_tool" and cvarTool and cvarTool:GetString() == mode) then return end
		-- check if we're looking at a valid entity
		...
		draw.SimpleTextOutlined(upwrd, "improved_stacker_direction", us.x, us.y, BLUE, 1, 0, 1, BLACK)
		cam.End2D()
	end)
end
...
if CLIENT then
	...
	local function buildCPanel(cpanel)
		...
		local presets = {
			...
			Options = {
				[L(prefix .. "combobox_default")] = {
					...
					["improved_stacker_freeze"] = "1",
					["improved_stacker_weld"] = "1",
					["improved_stacker_nocollide"] = "1",
					["improved_stacker_ghostall"] = "1",
					["improved_stacker_material"] = "1",
					["improved_stacker_physprop"] = "1",
					["improved_stacker_color"] = "1",
					["improved_stacker_offsetx"] = "0",
					["improved_stacker_offsety"] = "0",
					["improved_stacker_offsetz"] = "0",
					["improved_stacker_pitch"] = "0",
					["improved_stacker_yaw"] = "0",
					["improved_stacker_roll"] = "0",
					["improved_stacker_relative"] = "1",
					["improved_stacker_draw_halos"] = "0",
					["improved_stacker_halo_r"] = "255",
					["improved_stacker_halo_g"] = "0",
					["improved_stacker_halo_b"] = "0",
					["improved_stacker_halo_a"] = "255",
					["improved_stacker_draw_axis"] = "1",
					["improved_stacker_axis_labels"] = "1",
					["improved_stacker_axis_angles"] = "0",
				},
			},
			CVars = {"improved_stacker_mode", "improved_stacker_direction", "improved_stacker_count", "improved_stacker_freeze", "improved_stacker_weld", "improved_stacker_nocollide", "improved_stacker_ghostall", "improved_stacker_material", "improved_stacker_physprop", "improved_stacker_color", "improved_stacker_offsetx", "improved_stacker_offsety", "improved_stacker_offsetz", "improved_stacker_pitch", "improved_stacker_yaw", "improved_stacker_roll", "improved_stacker_relative", "improved_stacker_draw_halos", "improved_stacker_halo_r", "improved_stacker_halo_g", "improved_stacker_halo_b", "improved_stacker_halo_a", "improved_stacker_draw_axis", "improved_stacker_axis_labels", "improved_stacker_axis_angles",}
		}
		...
		cpanel:AddControl("Slider", {
			...
			Description = "How many props to create in each stack"
		})
		...
		cpanel:AddControl("Slider", {
			...
			Value = 0,
			Command = "improved_stacker_offsetx"
		})
		...
		cpanel:AddControl("Slider", {
			...
			Value = 0,
			Command = "improved_stacker_offsety"
		})
		...
		cpanel:AddControl("Slider", {
			...
			Value = 0,
			Command = "improved_stacker_offsetz"
		})
		...
		cpanel:AddControl("Slider", {
			...
			Max = MAX_ANGLE,
			Value = 0,
			Command = "improved_stacker_pitch"
		})
		...
		if showSettings then
			...
			cpanel:AddControl("Slider", {
				...
				Min = 0,
				Max = 255,
				Command = "improved_stacker_opacity"
			})
	...
	hook.Add("PopulateToolMenu", "improved_stackerAdminUtilities", function()
		spawnmenu.AddToolMenuOption("Utilities", "Admin", "improved_stacker_utils", L(prefix .. "name"), "", "", function(cpanel)
			...
			local presets = {
				...
				cvars = {
					{
						CVar = "improved_stacker_max_per_player",
						CCmd = "improved_stacker_set_max_per_player"
					},
					{
						CVar = "improved_stacker_max_per_stack",
						CCmd = "improved_stacker_set_max_per_stack"
					},
					{
						CVar = "improved_stacker_delay",
						CCmd = "improved_stacker_set_delay"
					},
					{
						CVar = "improved_stacker_max_offsetx",
						CCmd = "improved_stacker_set_max_offsetx"
					},
					{
						CVar = "improved_stacker_max_offsety",
						CCmd = "improved_stacker_set_max_offsety"
					},
					{
						CVar = "improved_stacker_max_offsetz",
						CCmd = "improved_stacker_set_max_offsetz"
					},
					{
						CVar = "improved_stacker_force_freeze",
						CCmd = "improved_stacker_set_force_freeze"
					},
					{
						CVar = "improved_stacker_force_weld",
						CCmd = "improved_stacker_set_force_weld"
					},
					{
						CVar = "improved_stacker_force_nocollide",
						CCmd = "improved_stacker_set_force_nocollide"
					},
					{
						CVar = "improved_stacker_force_stayinworld",
						CCmd = "improved_stacker_set_force_stayinworld"
					},
				},
			}
			...
			local sliders = {
				{
					String = "max_per_player",
					Min = -1,
					Max = 2048,
					Decimals = 0
				},
				{
					String = "max_per_stack",
					Min = 1,
					Max = 100,
					Decimals = 0
				},
			...
			for k, data in pairs(sliders) do
				...
				slider:SetValue(decimals == 0 and GetConVar("improved_stacker_" .. data.String):GetInt() or GetConVar("improved_stacker_" .. data.String):GetFloat(), true)
				local cmd = "improved_stacker_set_" .. data.String
			...
			local checkboxes = {"freeze", "weld", "nocollide", "nocollide_all", "stayinworld",}
